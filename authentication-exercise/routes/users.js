...
router.get("/:email",(req,res)=>{
  // Copy the code here
  let emailById = users.filter((user)=> {
      return user.email === req.params.email
  })
  res.send(emailById)
});

// Get by specific LastName request: Retrive a single user with LastName
router.get("/:LastName",(req,res)=>{
    let userByLastName = users.filter((user)=>{
        return user.lastName === req.params.LastName;
    })
    res.send(userByLastName)
})
function getDataFromString(strData){
    let [dd,mm,yyyy] = strDate.split("-")
    return new Date(yyyy+"/"+mm+"/"+dd);
};
// Sorting users by DOB
router.get("/sort",(req,res)=>{
    let sorted_users = users.sort((a,b)=>{
        let d1 = getDataFromString(a.DOB);
